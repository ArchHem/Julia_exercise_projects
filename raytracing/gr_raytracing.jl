...
rho2 = x2^2 + alpha^2 * cos(x4)^2
delta = x2^2 - r_s * x2 + alpha^2

g_00 = -(1 - r_s * x2 / rho2) * c^2
g_02 = - 2 * r_s * x2 * alpha * sin(x4)^2 / rho2 * c
g_11 = rho2/delta
g_22 = (x2^2 + alpha^2 + r_s * alpha^2 * x2 * sin(x4)^2 / rho2) * sin(x4)^2
g_33 = rho2
...
w = (x_i^2 + y_i^2 + z_i^2) - (alpha^2)

x1_i = t_i
x2_i = sqrt( (w + sqrt((w^2) + (4 * (alpha^2) * (z_i^2))))) / sqrt(2)
x3_i = atan(y_i, x_i)
x4_i = acos(z_i/x2_i)
...
@inline function calculate_fouracc_d0(metric_instance::metric_container,coord_fourpos::Vector{SVector{4, Float64}},
    ...
    Threads.@threads for n in 1:N_rays
        ...
        for u in 1:4
            local_acc[u] = -(local_veloc'CHR_symbol[u,:,:] * local_veloc) + (local_veloc'CHR_symbol[1,:,:] * local_veloc * local_veloc[u])
        end
...
@inline function calculate_fouracc_affine(metric_instance::metric_container,coord_fourpos::Vector{SVector{4, Float64}},
    coord_fourveloc::Vector{SVector{4, Float64}})
    N_rays = length(coord_fourpos)
    coord_four_acceleration = Vector{SVector{4, Float64}}(undef,N_rays)
    #hopefully matches einsum-like performance
...
function KERR_termination_cause(coord_fourpos::Vector{SVector{4, Float64}}, coord_fourveloc::Vector{SVector{4, Float64}},
    ...
    for i in 1:N_current
        if coord_fourpos[i][2] < outer_horizon * 1.025 || coord_fourpos[i][2] > 20.0
...
function integrate_geodesics_no_tracking_RK4_d0(metric_instance::metric_container,coord_fourpos::Vector{SVector{4, Float64}},
    coord_fourveloc::Vector{SVector{4, Float64}},termination::Function,d0_scaler::Function,
    N_timesteps::Int64 = 2000)
    ...
    for t in ProgressBar(1:N_timesteps)
        ...
        d1_fourpos, d1_fourveloc = calculate_fouracc_d0(metric_instance,coord_fourpos,coord_fourveloc)
        
        d2_fourpos, d2_fourveloc = calculate_fouracc_d0(metric_instance,
        coord_fourpos .+ 0.5 .* local_d0 .* d1_fourpos, coord_fourveloc .+ 0.5 .* local_d0 .* d1_fourveloc)

        d3_fourpos, d3_fourveloc = calculate_fouracc_d0(metric_instance,
        coord_fourpos .+ 0.5 .* local_d0 .* d2_fourpos, coord_fourveloc .+ 0.5 .* local_d0 .* d2_fourveloc)

        d4_fourpos, d4_fourveloc = calculate_fouracc_d0(metric_instance,
        coord_fourpos .+  local_d0 .* d3_fourpos, coord_fourveloc .+ local_d0 .* d3_fourveloc)
...
function integrate_geodesics_no_tracking_RK4_affine(metric_instance::metric_container,coord_fourpos::Vector{SVector{4, Float64}},
    coord_fourveloc::Vector{SVector{4, Float64}},termination::Function,d0_scaler::Function,
    N_timesteps::Int64 = 2000)
    ...
    for t in ProgressBar(1:N_timesteps)
        ...
        d1_fourpos, d1_fourveloc = calculate_fouracc_affine(metric_instance,coord_fourpos,coord_fourveloc)
        
        d2_fourpos, d2_fourveloc = calculate_fouracc_affine(metric_instance,
        coord_fourpos .+ 0.5 .* local_d0 .* d1_fourpos, coord_fourveloc .+ 0.5 .* local_d0 .* d1_fourveloc)

        d3_fourpos, d3_fourveloc = calculate_fouracc_affine(metric_instance,
        coord_fourpos .+ 0.5 .* local_d0 .* d2_fourpos, coord_fourveloc .+ 0.5 .* local_d0 .* d2_fourveloc)

        d4_fourpos, d4_fourveloc = calculate_fouracc_affine(metric_instance,
        coord_fourpos .+  local_d0 .* d3_fourpos, coord_fourveloc .+ local_d0 .* d3_fourveloc)
...
function SCH_colorer(final_fourvectors::Vector{SVector{4, Float64}},final_fourvelocs::Vector{SVector{4, Float64}},image::Matrix{RGBA{N0f8}})
    for i in eachindex(image)
        if final_fourvectors[i][2] < outer_horizon * 1.025
...
function standard_CS_renderer_d0(image_path::String, metric_instance::metric_container,coord_fourpos::Vector{SVector{4, Float64}},
    ...
    starting_fourpos, starting_fourveloc, final_fourpos, final_fourveloc = integrate_geodesics_no_tracking_RK4_d0(metric_instance,
    coord_fourpos,coord_fourveloc,termination,d0_scaler,N_timesteps)
...
function standard_CS_renderer_affine(image_path::String, metric_instance::metric_container,coord_fourpos::Vector{SVector{4, Float64}},
    coord_fourveloc::Vector{SVector{4, Float64}},termination::Function,d0_scaler::Function, N_x_cam::Int64, N_y_cam::Int64,custom_colorer::Function,
    N_timesteps::Int64 = 2000)
    ...
    starting_fourpos, starting_fourveloc, final_fourpos, final_fourveloc = integrate_geodesics_no_tracking_RK4_affine(metric_instance,
    coord_fourpos,coord_fourveloc,termination,d0_scaler,N_timesteps)
...
Nx = 100
Ny = 100
fourvec0, fourveloc0 = planar_camera_ray_generator(test_container,Nx,Ny,0.05,Vector([0.0,0.0,0.0,-5]),1.0,0.0,0.0,0.0)
